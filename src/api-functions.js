...
function makeErrorMessage() {
  ...
  errorMessage.textContent =
    'Enter search in the form of "City", "City, State", "City, Country", or "Zip Code"';
...
async function grabCurrentWeatherData(location) {
  const response = await fetch(
    `https://api.weatherapi.com/v1/current.json?key=3e2d011cbd1d4bf4996154952233105&q=${location}`,
    { mode: "cors" }
...
async function grabForecastWeatherData(location) {
  const response = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=3e2d011cbd1d4bf4996154952233105&q=${location}&days=7`,
    { mode: "cors" }
  );
